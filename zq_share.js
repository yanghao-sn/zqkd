...
[task_local]
#中青看点火爆转发
15 6,12,18 * * * https://github.com/JDWXX/ql_all/blob/master/zqkd/zq_share.js, tag=中青看点火爆转发, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jxcfd.png, enabled=true

================Loon==============
[Script]
cron "15 6,12,18 * * *" script-path=https://github.com/JDWXX/ql_all/blob/master/qsk/zq_share.js,tag=中青看点火爆转发

===============Surge=================
中青看点火爆转发 = type=cron,cronexp="15 6,12,18 * * *",wake-system=1,timeout=3600,script-path=https://github.com/JDWXX/ql_all/blob/master/qsk/zq_share.js

============小火箭=========
中青看点火爆转发 = type=cron,script-path=https://github.com/JDWXX/ql_all/blob/master/qsk/zq_share.js, cronexpr="15 6,12,18 * * *", timeout=3600, enable=true
*/
...
!(async () => {
     ...
     $.done()}else {
         ...
         for (let k = 0; k < zq_cookieArr.length; k++) {
             // $.message = ""
             //bodyVal2 =zq_cookie2.split('&token=')[0]
             ...
             if (hour > 4 && hour !== 10 && hour !== 16 && hour !== 22) {
                 ...
                 await $.wait(4000);
                 await sharejl()
                 //console.log(typeof(zq_cookie1));
                 //console.log(zq_cookie1.length.toString());
                 await $.wait(4000);
                 console.log("\n\n")
             } else {
                 console.log('\n现在不是转发时段！')
...
function share(wzid,timeout=0) {
    ...
    'Host': 'kandian.wkandian.com',
     'Referer': 'https://kandian.wkandian.com/h5/20200612makeMoney/?' +zq_cookie1
},
            body :  zq_cookie1 + '&article_id='+wzid,}
        $.post(url, async (err, resp, data) => {
            try {
...
function sharejl(timeout=0) {
    ...
    'Host': 'kandian.wkandian.com',
     'Referer': 'https://kandian.wkandian.com/h5/20200612makeMoney/?' +zq_cookie1
            ...
            body :  zq_cookie1 + '&action=beread_extra_reward_'+ reward,}
        $.post(url, async (err, resp, data) => {
            try {
